...
def generator(set, common_length: int, WD = os.getcwd()):
    ...
    c=0
    for pandas_dtf in glob(f'{WD}/database_sequences/data/{set}/pand_Escherichiacoli'):
...
def training_process(nameoutput, parameters=None):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch > 0 and epoch % 1 == 0:  # Save parameters every 10 epochs
            ...
            tsave(model.state_dict(), f"network/scripts/CaNAT/parameters_fine_tuning{nameoutput}.pt")

            with open(f"network/scripts/CaNAT/losses_fine_tune_{nameoutput}.txt", 'w') as f:
